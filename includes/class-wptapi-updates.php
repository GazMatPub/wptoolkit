...
class WPToolKit_Updates {
			...
			if ($wptoolkit_plugins = get_option('wptoolkit_plugins')) {
				...
				foreach($all_plugins as $key => $plugin) {
					...
					if (array_key_exists($key, $wptoolkit_plugins)) {
						...
						$plugin_url = 'http://api.wptoolkit.com/?wpt_plugin_download=get&plugin_id='.$wptoolkit_plugins[$key]['plugin_id'].'&email='.$email.'&licence_key='.$licence_key.'&product_id='.$product_id.'&instance='.$instance.'&request=wptoolkit_status';
			...
			if ($wptoolkit_themes = get_option('wptoolkit_themes')) {
				foreach($all_themes as $key => $theme) {
					...
					if (array_key_exists($key, $wptoolkit_themes)) {
						...
						$theme_url = 'http://api.wptoolkit.com/?wpt_theme_download=get&theme_id='.$wptoolkit_themes[$key]['theme_id'].'&email='.$email.'&licence_key='.$licence_key.'&product_id='.$product_id.'&instance='.$instance.'&request=wptoolkit_status';
	...
	public static function get_plugin_catalogue() { 
		$url = 'http://api.wptoolkit.com/?wptoolkit_repo=json&type=all';
