...
if ( get_option( 'wptoolkit_plugin_manager_activated' ) != 'Activated' ) {
    add_action( 'admin_notices', 'WPToolKit_Plugin_Manager::wpt_inactive_notice' );
}

class WPToolKit_Plugin_Manager {
	 ...
	 */
	public $wptoolkit_plugin_manager_version_name = 'plugin_wptoolkit_plugin_manager_version';
	 ...
	 * @var string
	 */
	public $text_domain = 'wptoolkit-plugin-manager';
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			 ...
			 */
			$this->ame_software_product_id = 'WPToolKit Plugin Manager';

			/**
			 * Set all data defaults here
			 */
			$this->ame_data_key 				= 'wptoolkit_plugin_manager';
			$this->ame_api_key 					= 'api_key';
			$this->ame_activation_email 		= 'activation_email';
			$this->ame_product_id_key 			= 'wptoolkit_plugin_manager_product_id';
			$this->ame_instance_key 			= 'wptoolkit_plugin_manager_instance';
			$this->ame_deactivate_checkbox_key 	= 'wptoolkit_plugin_manager_deactivate_checkbox';
			$this->ame_activated_key 			= 'wptoolkit_plugin_manager_activated';

			/**
			 * Set all admin menu data
			 */
			$this->ame_deactivate_checkbox 			= 'am_deactivate_example_checkbox';
			$this->ame_activation_tab_key 			= 'wptoolkit_settings_page';
			$this->ame_deactivation_tab_key 		= 'wptoolkit_plugin_manager_deactivation';
			$this->ame_settings_menu_title			= 'WP Toolkit Dashboard';
	 ...
	 * API Key Class.
	 *
	 * @return WPToolKit_Plugin_Manager_Key
	 */
	public function key() {
		return WPToolKit_Plugin_Manager_Key::instance();
	}

	/**
	 * Update Check Class.
	 *
	 * @return WPToolKit_Plugin_Manager_Update_API_Check
	 */
	public function update_check( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '' ) {
		...
		return WPToolKit_Plugin_Manager_Update_API_Check::instance( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra );
	}
	...
	public function activation() {
		...
		if ( version_compare( $this->version, $curr_ver, '>' ) ) {
			// update the version
			update_option( $this->wptoolkit_plugin_manager_version_name, $this->version );
		}
     ...
     */
	public static function wpt_inactive_notice() { ?>

		<?php if ( ! current_user_can( 'manage_options' ) ) return; ?>
		<?php if ( isset( $_GET['page'] ) && 'wptoolkit_settings_page' == $_GET['page'] ) return; ?>
		<div id="message" class="error">
			<p><?php printf( __( 'WP Toolkit Plugin Manager has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.', 'wptoolkit-plugin-manager' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wptoolkit_settings_page' ) ) . '">', '</a>' ); ?></p>
		</div>
		<?php
	}
	...
	public function check_external_blocking() {
		...
		if( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
					<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'wptoolkit-plugin-manager' ), $this->ame_software_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?></p>
				</div>
				<?php
			}
...
function WPT() {
    return WPToolKit_Plugin_Manager::instance();
}
